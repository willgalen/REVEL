...
/** Factory for creating objects of any type by invoking their primary constructor.
  * Unlike Java reflection Methods or Scala reflection Mirrors, this factory
  * can be safely passed along with tasks that must be serializable. */
class AnyObjectFactory[T: TypeTag] extends Logging with Serializable {
  ...
  private def resolveDirectOuterInstance() = {
    ...
    // this is for external shells such as the Spark Shell - all classes created in the console are inner classes of some mysterious Spark classes;
...
object AnyObjectFactory extends Logging {
   ...
   * [[com.datastax.driver.scala.core.utils.AnyObjectFactory#isRealMemberClass isRealMemberClass]] returns `true`.
   */

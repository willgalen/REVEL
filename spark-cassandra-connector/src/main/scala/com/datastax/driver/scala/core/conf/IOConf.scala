...
/** Read settings
  ...
  */
case class ReadConf(splitSize: Int, fetchSize: Int, consistencyLevel: ConsistencyLevel) extends IOConf

object ReadConf {
  import Read._

  final val Default = ReadConf(DefaultSplitSize, DefaultFetchSize, DefaultConsistencyLevel)
...
object WriteConf {
  ...
  final val Default = WriteConf(BatchSize.Automatic, DefaultConsistencyLevel, DefaultParallelismLevel)

...
object TableDef {
  def apply(connector: CassandraCluster, keyspaceName: String, tableName: String): TableDef = {
...
object Schema extends Logging {
  ...
  /** Fetches database schema from Cassandra. Provides access to keyspace, table and column metadata.
    ...
    */
  def fromCassandra(cluster: CassandraCluster, keyspaceName: Option[String] = None, tableName: Option[String] = None): Schema = {

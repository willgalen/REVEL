...
import com.datastax.driver.scala.testkit.{AbstractFlatSpec, CassandraSpec}

class CassandraPartitionKeyWhereSpec extends AbstractFlatSpec with CassandraSpec {
  import com.datastax.driver.scala.core._

  useCassandraConfig("cassandra-default.yaml.template")

  CassandraCluster(cassandraHost).withSessionDo { session =>
  ...
  "A CassandraRDD" should "allow partition key eq in where" in {
    val table = cassandra.table("where_test", "key_value").where("key = ?", 1)
    val result = table.stream.toList
    result should have length 1
    result.head.getInt("key") should be (1)
  }

  it should "allow partition key 'in' in where" in {
    val result = cassandra.table("where_test", "key_value").where("key in (?, ?)", 2,3).stream.toList
    result should have length 2
    result.head.getInt("key") should (be (2) or be (3))
  }

  it should "allow cluster key 'in' in where" in {
    val result = cassandra.table("where_test", "key_value").where("group in (?, ?)", 200,300).stream.toList
    result should have length 2
    result.head.getInt("key") should (be (2) or be (3))
  }

  it should "work with composite keys in" in {
    val result = cassandra.table("where_test", "ckey_value").where("key1 = 1 and \"Key2\" in (?, ?)", 100,200).stream.toList
    result should have length 1
    result.head.getInt("key1") should be (1)
  }

  it should "work with composite keys eq" in {
    val result = cassandra.table("where_test", "ckey_value").where("key1 = ? and \"Key2\" = ?", 1,100).stream.toList
    result should have length 1
    result.head.getInt("key1") should be (1)
  }

  it should "work with composite keys in2" in {
    val result = cassandra.table("where_test", "ckey_value").where("\"Key2\" in (?, ?) and key1 = 1", 100,200).stream.toList
    result should have length 1
    result.head.getInt("key1") should be (1)
  }
}

...
final class CassandraSettings private(val alternate: Source, prefix: Option[String], useDefaults: Boolean = false) extends FallbackSettings {
  ...
  val CassandraHosts: Set[InetAddress] =
    get(Connection.HostProperty).getOrElse(InetAddress.getLocalHost.getHostAddress)
     .split(",").toSet[String].map(InetAddressFromString(_))
  val NativePort: Int = toInt(Connection.PortProperty, Connection.DefaultNativePort)
  val RpcPort: Int = toInt(Connection.RpcPortProperty, Connection.DefaultRpcPort)
...
object Connection {
  ...
  val PortProperty = "cassandra.connection.native.port"

...
class TableWriterSpec extends SparkCassandraITFlatSpecBase {
  ...
  conn.withSessionDo { session =>
    ...
    session.execute("CREATE TABLE IF NOT EXISTS write_test.collections_mod (key INT PRIMARY KEY, lcol list<text>, scol set<text>, mcol map<text, text>)")
  ...
  it should "be able to append and prepend elements to a C* list" in {
    ...
    listElements.saveToCassandra("write_test", "collections_mod", SomeColumns("key", "lcol" append))
    prependElements.saveToCassandra("write_test", "collections_mod", SomeColumns("key", "lcol" prepend))

    val testList = sc.cassandraTable[(Seq[String])]("write_test", "collections_mod")
      .where("key = 1")
  ...
  it should "be able to remove elements from a C* list " in {
    ...
    sc.parallelize(Seq(
      (2,Vector("Two")),
      (2,Vector("Three"))))
      .saveToCassandra("write_test", "collections_mod", SomeColumns("key", "lcol" remove))
  ...
  it should "be able to add elements to a C* set " in {
    val setElements = sc.parallelize(Seq(
      ...
      (3,Set("Two")),
      (3,Set("Three"))))
    setElements.saveToCassandra("write_test", "collections_mod", SomeColumns("key", "scol" append))
    val testSet = sc.cassandraTable[(Set[String])]("write_test", "collections_mod")
      .where("key = 3")
  ...
  it should "be able to remove elements from a C* set" in {
    ...
    sc.parallelize(Seq((4,Set("Two")), (4,Set("Three"))))
      .saveToCassandra("write_test", "collections_mod", SomeColumns("key", "scol" remove))
  ...
  it should "be able to add key value pairs to a C* map" in {
    val setElements = sc.parallelize(Seq(
      ...
      (5,Map("Two" -> "Two")),
      (5,Map("Three" -> "Three"))))
    setElements.saveToCassandra("write_test", "collections_mod", SomeColumns("key", "mcol" append))

    val testMap = sc.cassandraTable[(Map[String, String])]("write_test", "collections_mod")
      .where("key = 5")
  ...
  it should "throw an exception if you try to apply a collection behavior to a normal column" in {
    ...
    val e = intercept[IllegalArgumentException] {
      sc.parallelize(col).saveToCassandra("write_test", "key_value_1", SomeColumns("key", "group" overwrite, "value"))
    }
  ...
  it should "throw an exception if you try to remove values from a map" in {
    val setElements = sc.parallelize(Seq(
      ...
      (5,Map("Three" -> "Three"))))
    val e = intercept[IllegalArgumentException] {
      setElements.saveToCassandra("write_test", "collections_mod", SomeColumns("key", "mcol" remove))
    }
  ...
  it should "throw an exception if you prepend anything but a list" in {
    ...
    val e = intercept[IllegalArgumentException] {
      setElements.saveToCassandra("write_test", "collections_mod", SomeColumns("key", "mcol" prepend, "scol" prepend))
    }
    e.getMessage should include ("mcol")

package com.datastax.driver.scala.embedded
...
object EmbeddedCassandra {
  ...
  private[datastax] var cassandra: Option[CassandraRunner] = None

  private[datastax] var currentConfigTemplate: String = null

 // val cassandraHost = sys.env.get(HostProperty).map(InetAddress.getByName(_)) getOrElse DefaultHost
...
private[datastax] class CassandraRunner(val configTemplate: String) extends Embedded {
  ...
  //if (!waitForPortOpen(DefaultHost, DefaultNativePort, 10000))

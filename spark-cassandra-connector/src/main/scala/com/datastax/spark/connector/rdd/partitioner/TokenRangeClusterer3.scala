...
class TokenRangeClusterer3[V, T <: Token[V]](maxRowCountPerGroup: Long, maxGroupSize: Int = Int.MaxValue) {
  ...
  private def group(tokenRanges: Stream[TokenRange[V, T]],
                    result: Vector[Seq[TokenRange[V, T]]])(implicit partitionSize: Int): Iterable[Seq[TokenRange[V, T]]] = {
  ...
  def group(tokenRanges: Seq[TokenRange[V, T]]): Iterable[Seq[TokenRange[V, T]]] = {
    ...
    val rowCounts = for (tr ← tokenRanges; rc ← tr.rowCount) yield rc

    val rowCountPerNode = rowCounts.sum.toDouble / trReplicas.toSet.size.toDouble

    val splitsPerNode = Math.ceil(rowCountPerNode / maxRowCountPerGroup.toDouble)

    val rowCountPerPartition = Math.ceil(rowCountPerNode / splitsPerNode) + 1
    ...
    group(sortedRanges.toStream, Vector.empty)(rowCountPerPartition.toInt)
  }

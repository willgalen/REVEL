...
import com.datastax.driver.scala.core.conf.{CassandraSettings, WriteConf}
import com.datastax.driver.scala.testkit.{AbstractSpec, EmbeddedCassandraFixture}

class TableWriterColumnNamesSpec extends AbstractSpec with EmbeddedCassandraFixture {

  implicit val settings = CassandraSettings(useDefaults = true)
  ...
  "TableWriter" must {
    ...
    "fail in the RowWriter if provided specified column names do not include primary keys" in {
      intercept[IllegalArgumentException] {
        Seq((1, 1L, None)).write("column_names_test", "key_value", SomeColumns("key", "value"))
      }
    }

...
import com.datastax.driver.scala.core.conf.{CassandraConnectorConf, CassandraSettings, ReadConf}
import com.datastax.driver.scala.core.utils.{Logging, CountingIterator}
import com.datastax.driver.scala.core.{Schema, Connector}
...
class TableReader[R] private[datastax] (val connector: Connector,
  ...
  private def copy(columnNames: ColumnSelector = columnNames,
                   ...
                   readConf: ReadConf = readConf, connector: Connector = connector): TableReader[R] =
  ...
  def withConnector(connector: Connector): TableReader[R] =
  ...
  private def fetchTokenRange(session: Session, range: CqlTokenRange): Iterator[R] = {
    ...
    try {
      ...
      // TODO with akka.stream.FlowMaterializer, akka.stream.scaladsl.{Sink, Source} on spark upgrade
  ...
  def stream: Stream[R] = {
    ...
    val rowIterator = fetchTokenRange(session, CqlTokenRange(""))// tokenRanges.iterator.flatMap(fetchTokenRange(session, _))
...
object TableReader {
  ...
  def apply[T](conf: CassandraConnectorConf, keyspaceName: String, tableName: String, readConf: ReadConf)
              (implicit ct: ClassTag[T], rrf: RowReaderFactory[T]): TableReader[T] =
    new TableReader[T](Connector(conf), keyspaceName, tableName, AllColumns, CqlWhereClause.empty, readConf)

  def apply[K, V](conf: CassandraConnectorConf, keyspaceName: String, tableName: String, readConf: ReadConf)
                 (implicit keyCT: ClassTag[K], valueCT: ClassTag[V], rrf: RowReaderFactory[(K, V)]): TableReader[(K, V)] =
    new TableReader[(K, V)](Connector(conf), keyspaceName, tableName, AllColumns, CqlWhereClause.empty, readConf)
}

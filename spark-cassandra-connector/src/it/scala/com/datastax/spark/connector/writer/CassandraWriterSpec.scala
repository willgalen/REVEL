...
class CassandraWriterSpec extends FlatSpec with Matchers with BeforeAndAfter with SharedEmbeddedCassandra with SparkTemplate {
  ...
  it should "write RDD of case class objects applying proper data type conversions" in {
    val col = Seq(
      KeyValueWithLongConversion("1", 1, "value1"),
      KeyValueWithLongConversion("2", 2, "value2"),
      KeyValueWithLongConversion("3", 3, "value3")
    )

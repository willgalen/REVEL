...
import com.datastax.driver.scala.core.utils.{Reflection, AnyObjectFactory}
import com.datastax.driver.scala.mapping.ColumnMapper
import com.datastax.driver.scala.types.{TypeConversionException, TypeConverter}
...
class ClassBasedRowReader[R : TypeTag : ColumnMapper](table: TableDef, skipColumns: Int = 0) extends RowReader[R] {
  ...
  private[datastax] val factory = new AnyObjectFactory[R]
...
class ClassBasedRowReaderFactory[R : TypeTag : ColumnMapper] extends RowReaderFactory[R] {
  ...
  override def targetClass: Class[R] = Reflection.getRuntimeClass(typeTag[R])
}

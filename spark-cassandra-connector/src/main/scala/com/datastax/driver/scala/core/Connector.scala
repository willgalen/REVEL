...
/** Provides and manages connections to Cassandra.
  *
  * A `Connector` instance is serializable and can be safely sent over network,
  ...
  *
  * Multiple `Connector`s in the same JVM connected to the same Cassandra cluster
  * will share a single underlying `Cluster` object. `Connector` will close the
  ...
  *
  * A `Connector` object is configured from [[CassandraConnectorConf]] object which
  ...
  */
class Connector(conf: CassandraConnectorConf) extends Serializable with Logging {
...
object Connector extends Logging {
  ...
  /** Returns a Connector. */
  def apply(settings: CassandraSettings): Connector =
    apply(CassandraConnectorConf(settings))

  def apply(conf: CassandraConnectorConf): Connector =
    new Connector(conf)

  /** Returns a Connector created from defaults with the `host` and `authConf`. */
  def apply(host: InetAddress, auth: AuthConf = NoAuthConf): Connector =
    apply(CassandraConnectorConf.Default.copy(hosts = Set(host), authConf = auth))
